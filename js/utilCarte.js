...
function initialize() 
{
	...
	var map = new google.maps.Map(document.getElementById('map_canvas'),mapOptions);
	 setMarkers(map, villes, imgStation, txtStation);
	var img = "cercle.png";
	...
	var optionsCircle2 = {
        center: new google.maps.LatLng(48.88057012853454, 2.352447509765625),
        map: map,
        radius: 0,
        strokeColor: '#2aa750',
        strokeOpacity: 0.8,
		...
		//Pour styliser la carte on a besoin de ces deux variables.
		var styles =   [
		...
		/*//Deux vars pour afficher une image comme icone.
		var myLatLng=new google.maps.LatLng(48.86057012853454, 2.352447509765625);
		var marker=new google.maps.Marker({position:myLatLng,map:map,icon:img,title:'ImagedeTest'});
		
		agrandirCercle(myCircle, 300);
		agrandirCercle(myCircle2, 300);
		*/
...
function nextStation(){
	...
	var city = villes[indice];
	var photo = imgStation[indice];
	var hist = txtStation[indice];
	showDiv(city[0],photo[0],hist[0]);
...
function hideDiv() { 
	...
	function showDiv(villes, img, textStation) {
	...
	function setMarkers(map, locations, station,txtStation) {
			...
			for (var i = 0; i < locations.length; i++) {
				...
				(function(i) {
					google.maps.event.addListener(marker, "click", function() {
						...
						indice = i;
						showDiv(villes[0],img[0],textStation[0]);
					});
				})(i);
			}
	}